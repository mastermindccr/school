...
void *malloc(int size){
    ...
    Block *ret = NULL;
    while(cur!=NULL){
        if((!cur->used && cur->size>=size && (ret==NULL || cur->size<ret->size))){
            ret = cur;
        }
        cur = cur->next;
    }

    if(ret->size-size>32){ // able to be the next new chunk
        Block* newBlock = (Block*)((void*)ret + 32 + size);
        newBlock->size = ret->size - size - 32;
        newBlock->used = 0;
