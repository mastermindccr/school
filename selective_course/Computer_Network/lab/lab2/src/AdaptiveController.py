...
class SimpleController(app_manager.RyuApp):
	...
	for num in range(3,0,-1):
	    for stat in sorted([flow for flow in body if flow.priority==num], 
                key=lambda flow: (flow.match['in_port'])):
                    if ev.msg.datapath.id == 2 and stat.match['ipv4_dst']=='10.0.0.1':
                        print("switch %d of rule %d : count %d packets, bandwidth: %f" % (ev.msg.datapath.id,num,stat.packet_count,stat.packet_count/5))
