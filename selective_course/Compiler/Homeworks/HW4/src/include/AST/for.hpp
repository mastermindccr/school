  ...
  public:
    ...
    ForNode(const uint32_t line, const uint32_t col,
        ...
        : AstNode{line, col}, m_loop_var_decl(p_loop_var_decl),
          ...
          m_body(p_body) {
            for(auto &var: m_loop_var_decl->getVariables())
                var->setType("loop_var");
          }
    pair<int, int> getLoopVarRange(){
        int start = atoi(static_cast<ConstantValueNode*>(m_init_stmt->getExprNode())->getConstantValueCString());
        int end = atoi(static_cast<ConstantValueNode*>(m_end_condition.get())->getConstantValueCString());
