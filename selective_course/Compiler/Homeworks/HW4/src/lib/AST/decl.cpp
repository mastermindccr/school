...
void DeclNode::init(const std::vector<IdInfo> *const p_ids,
    ...
    auto make_variable_node_and_emplace_back_in_var_nodes =
        [&](const IdInfo &id_info) {
            VariableNode* var_node = new VariableNode(id_info.location.line, id_info.location.col,
                                 id_info.id, p_type, shared_constant);
            if(shared_constant != nullptr)
                var_node->setType("constant");
            else
                var_node->setType("variable");
            m_var_nodes.emplace_back(var_node);
        };
