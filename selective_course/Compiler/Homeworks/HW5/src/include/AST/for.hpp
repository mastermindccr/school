...
class ForNode final : public AstNode {
  ...
  public:
    ...
    const DeclNode &getLoopVarDecl() { return *m_loop_var_decl.get(); };
    const AssignmentNode &getInitStmt() { return *m_init_stmt.get(); };
    const CompoundStatementNode &getBody() { return *m_body.get(); };
