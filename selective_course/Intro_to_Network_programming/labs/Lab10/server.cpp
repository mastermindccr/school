...
int main(int argc, char* argv[]){
    ...
    xdp* xdp_write = (xdp*) (datagram+sizeof(iphdr));
    strcpy(xdp_write->payload, "ok");

    while(recvfrom(s, receive, 1500, 0, (struct sockaddr*) &serv_addr, &addr_len)){
        xdp* xdp_h = (xdp*) (receive+sizeof(iphdr));
        ofstream file;
        file.open(argv[1]+stringify(xdp_h->filenum), ios::app);
        file.write(xdp_h->payload, strlen(xdp_h->payload));
        file.close();
